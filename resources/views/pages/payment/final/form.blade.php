...
@section('title', 'Pelunasan Pembayaran')
        ...
        <div class="col-md-10">
            ...
            <div class="card shadow-sm border-0 rounded-3 overflow-hidden">
                <div class="card-header bg-primary text-white py-3">
                    <h4 class="mb-0"><i class="fas fa-money-bill-wave me-2"></i>Pelunasan Pembayaran Booking #{{ $booking->id }}</h4>
                </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        <label class="text-muted small">DP yang Sudah Dibayar</label>
                                        <p class="fw-bold fs-5 text-success mb-0">Rp {{ number_format($dpAmount, 0, ',', '.') }}</p>
                                    </div>
                                    <div class="mb-3">
                                        <label class="text-muted small">Sisa Pembayaran</label>
                                        <p class="fw-bold fs-5 text-primary mb-0">Rp {{ number_format($remainingAmount, 0, ',', '.') }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="progress mt-2" style="height: 10px;">
                                @php
                                    $progressPercent = ($dpAmount / $booking->service->base_price) * 100;
                                @endphp
                            ...
                            <div class="d-flex justify-content-between mt-2">
                                <small class="text-muted">DP dibayar ({{ number_format($progressPercent, 0) }}%)</small>
                                <small class="text-muted">Pelunasan ({{ number_format(100 - $progressPercent, 0) }}%)</small>
                            </div>
                        </div>
                    </div>

                    <form action="{{ route('payment.final.process', $booking->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                        @csrf
                        
                        <div class="card mb-4 border-0 shadow-sm">
                            <div class="card-header bg-light d-flex align-items-center">
                                <i class="fas fa-credit-card text-primary me-2"></i>
                                <h5 class="mb-0">Detail Pelunasan</h5>
                            </div>
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-6">
                                        <label for="amount" class="form-label fw-bold">Jumlah Pelunasan</label>
                                        <div class="input-group mb-1">
                                            <span class="input-group-text">Rp</span>
                                            <input type="number" class="form-control form-control-lg @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ $remainingAmount }}" readonly required>
                                            @error('amount')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="form-text"><i class="fas fa-info-circle"></i> Jumlah pelunasan ditetapkan sebesar 50% dari total biaya.</div>
                                    </div>
                        ...
                        <div class="card mb-4 border-0 shadow-sm">
                            ...
                            <div class="card-body">
                                <p class="mb-3">Silakan transfer pelunasan ke salah satu rekening berikut:</p>
                        ...
                        <div class="d-grid gap-2 mt-4">
                            <button type="submit" class="btn btn-primary btn-lg py-3"><i class="fas fa-paper-plane me-2"></i>Kirim Bukti Pelunasan</button>
